...
for mm in range(0,n_months):
    ...
    FWI_resample[mm,:-1,:]= np.max(FWI[month_mask,:,-130:],axis=0)
    ISI_resample[mm,:-1,:]= np.max(ISI[month_mask,:,-130:],axis=0)
    BUI_resample[mm,:-1,:]= np.max(BUI[month_mask,:,-130:],axis=0)
    FFMC_resample[mm,:-1,:]= np.max(FFMC[month_mask,:,-130:],axis=0)
    DMC_resample[mm,:-1,:]= np.max(DMC[month_mask,:,-130:],axis=0)
    DC_resample[mm,:-1,:]= np.max(DC[month_mask,:,-130:],axis=0)
    """
...
plt.tight_layout()
plt.savefig('RandomForest_comparison_of_fire_risk_mexico.png')
plt.show()
