...
path2met = '/disk/scratch/local.2/dmilodow/ERAinterim/source_files/0.25deg_Indonesia/'
...
for mm in range(0,n_months):
    ...
    FWI_resample[mm,:,1:burned_area.shape[2]-1] = np.max(FWI[month_mask,:-1,:],axis=0)
    ISI_resample[mm,:,1:burned_area.shape[2]-1] = np.max(ISI[month_mask,:-1,:],axis=0)
    BUI_resample[mm,:,1:burned_area.shape[2]-1] = np.max(BUI[month_mask,:-1,:],axis=0)
    FFMC_resample[mm,:,1:burned_area.shape[2]-1] = np.max(FFMC[month_mask,:-1,:],axis=0)
    DMC_resample[mm,:,1:burned_area.shape[2]-1] = np.max(DMC[month_mask,:-1,:],axis=0)
    DC_resample[mm,:,1:burned_area.shape[2]-1] = np.max(DC[month_mask,:-1,:],axis=0)
    """
...
# population density
pop_file = '/home/dmilodow/DataStore_GCEL/WorldPop/Population/Asia_PPP_2015_adj_v2.tif'
...
burned_area = np.load(burned_area_file)['burned_area']
FWI_vars = np.load(FWI_predictors_file)['arr_0']
null_vars = np.load(null_predictors_file)['arr_0']
GLEAM_vars = np.load(GLEAM_predictors_file)['arr_0']
...
plt.tight_layout()
plt.savefig('RandomForest_comparison_of_fire_risk_indonesia.png')
plt.show()
